...
export async function deleteEmailAttachments(
	...
	try {
		...
		const attachmentList = await r2.list({ prefix });

		if (attachmentList.objects.length === 0) {
			return { success: true, deletedCount: 0 };
		}

		// Delete all objects with this prefix
		const deletePromises = attachmentList.objects.map((obj) => r2.delete(obj.key));
		await Promise.all(deletePromises);

		return { success: true, deletedCount: attachmentList.objects.length };
	} catch (error) {
		return {
			success: false,
			deletedCount: 0,
